...
* To output log records for one of the defined named loggers, either the
  ...
  other tools like clis to create the pywbem known logs with minimal work
  from command line or config file input.

  create_logger creates one or more loggers from parameter inputs that
  define the component name and characteristics of each logger.

* Also, loggers can be created by calling the logging  methods directly as
   in this example.To output the log records for all cim operations to
   ``stdout`` in a particular format, do this:
